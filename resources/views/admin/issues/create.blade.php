<x-app-layout>
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <select id="project" name="project">
                                ...
                                @if(\App\Models\Issue::where('id',$issue->id)->pluck('project_id')->first() == $name->id)
    ...
    <script type="text/javascript">
        ...
        function getId(user_id, issue_id) {
            var x = confirm("Are you sure you want to delete?");
            if (x) {
                var token = $("meta[name='csrf-token']").attr("content");
                var APP_URL = {!! json_encode(url('/')) !!};
                $.ajax({
                    url: APP_URL+"/issueuser/delete/" + issue_id + '/' + user_id,
                    type: 'DELETE',
                    data: {
                        "issue_id": issue_id,
                        "user_id": user_id,
        ...
        $("#deleteRecord_" + id).click(function() {
            ...
            if (x) {
                ...
                $.ajax({
                    url: "http://127.0.0.1:8000/user-issues/delete/" + issue_id + '/' + user_id,
                    type: 'DELETE',
                    data: {
                        "issue_id": issue_id,
                        "user_id": user_id,
